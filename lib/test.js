...
const tester = (collect, {offset = 0} = {}) => (description, spec) => {
	...
	const handleDelegate = async delegate => {
		...
		// Delegate is exhausted: create a summary tester point in the stream and throw the delegate
		if (done === true) {
	...
	// Execute the tester collecting assertions
	const assertFn = assert(collector, subTest);
	const task = new Promise(resolve => resolve(spec(assertFn)))
		.then(() => {
			// Always report a plan and summary: the calling tester will know how to deal with it
		...
		.catch(err => {
			// We report a failing tester before bail out ... while unhandled promise rejection is still allowed by nodejs...
	...
	// Collection by the calling tester
	collect(instance);
