...
const bindAssert = (assert, {onResult}) => Object.fromEntries(
    [...Object.keys(AssertPrototype)]
        .map((methodName) => [methodName, function (...args) {
            return onResult(assert[methodName](...args));
        }])
);

export default ({onResult = noop} = {
    onResult: noop
}) => {
    ...
    return bindAssert(Object.create(AssertPrototype), {onResult: resultCallback});
}
